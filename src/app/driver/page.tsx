...
const isLocationFresh = (driver: Driver) => {
  if (!driver.lastActiveAt) return false
  const lastActiveAt = Date.parse(driver.lastActiveAt)
  if (Number.isNaN(lastActiveAt)) return false
  if (driver.createdAt) {
    const createdAt = Date.parse(driver.createdAt)
    if (!Number.isNaN(createdAt) && lastActiveAt <= createdAt) {
      return false
    }
  }
  return Date.now() - lastActiveAt <= LOCATION_STALE_MS
}
...
export default function DriverApp() {
  ...
  const updateLocation = useCallback(
    async (latitude: number, longitude: number) => {
      ...
      try {
        ...
        setDriver((prev) => (prev ? { ...prev, ...updated } : updated))
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (!isLocationFresh(driver)) return
  ...
  const updateStatus = async (
    ...
    try {
      ...
      const locationPayload =
        driverLocation && locationStatus === 'granted'
          ? { latitude: driverLocation.lat, longitude: driverLocation.lng }
          : {}
      const updated = await fetchJson<Driver>(`/api/drivers/${driverId}`, {
        method: 'PUT',
        body: JSON.stringify({ status, ...locationPayload })
      })
      setDriver(updated)
  ...
  const locationDetail = !driverId
    ...
    : locationStatus === 'granted'
      ? 'Sharing your live position with dispatch, even when offline.'
      : locationStatus === 'denied'
        ? 'Permission blocked. Enable location in your browser settings.'
        : 'Enable location to stay visible on the live map.'
  ...
  useEffect(() => {
    ...
    requestLocationPermission()
  }, [driverId, isRequestingLocation, locationStatus, requestLocationPermission])
  ...
  }, [driverId, locationStatus, updateLocation])
          ...
          <CardContent className="p-6">
            ...
            {effectiveStatus === 'ONLINE' && !activeOrder && (
              <div className="mt-6 space-y-2">
                ...
                {!driverLocation && (
                  <p className="text-xs text-white/50">
                    Enable location sharing to stay visible, even offline.
                  </p>
                )}
              </div>
            )}
          ...
          <Card className="neon-card text-white">
            ...
            <CardContent className="space-y-5 p-6">
              <div className="space-y-2">
                ...
                {!driverLocation && (
                  <p className="text-xs text-white/50">
                    Enable location sharing to draw your live route and stay visible.
                  </p>
                )}
              </div>
    ...
    <div className="driver-shell relative min-h-screen overflow-hidden bg-[#050207] text-white">
      <style jsx global>{`
        ...
        .neon-panel {
          ...
          border: 1px solid var(--lip-border, rgba(255, 92, 170, 0.32));
